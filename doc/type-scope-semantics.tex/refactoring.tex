...
We highlight these similarities in~\cref{ren}, focusing only on \AIC{`var},
\AIC{`app}, and \AIC{`lam} only for the moment as they allow us to make all
...
to observe that the functor \AF{□} is a comonad: \AF{extract} applies
...
the (used but not shown) definitions \AF{map\textasciicircum{}Wrap} (functoriality)
and \AF{th\textasciicircum{}Wrap} (thinnability).
%
