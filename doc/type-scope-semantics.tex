...
\begin{enumerate}
  \item
    ...
    (\AB{⧫} \AB{σ} \AB{Γ}) into a term (\AD{Tm} \AB{σ} \AB{Γ)}.
  \item
    One should be able to craft a fresh environment value
    associated to the zeroth variable (\AB{⧫} \AB{σ} (\AB{σ} \AIC{∷} \AB{Γ}))
  \item
    One should be able to embed environment values defined in a
    scope \AB{Γ} into ones in a scope extended with a fresh variable
    (\AB{σ} \AIC{∷} \AB{Γ}).
\end{enumerate}
...
provided an environment of \AB{⧫}-values will turn a term into another
...
\end{minipage}
\caption{Renaming and Substitution as \AR{Kit}s}
\end{figure}
