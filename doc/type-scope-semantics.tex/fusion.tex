...
to the their being \AF{PER}-related. The canonical cases are trivially solved by
...
Formally (\AB{t} is a term, γ a substitution and ρ an evaluation environment):
...
\cref{sec:fusionrennbe}. We start by defining a notation \AF{𝓡} to lighten
the types, then combinators \AF{APPᴿ} and \AF{IFTEᴿ}. The cases for
...
(\cref{renevalfun}) and multiple uses of \AF{Eval\textasciicircum{}Sim}
(\cref{fig:nbeselfsim}).
