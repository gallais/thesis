...
We once again define \AF{𝓡}, a specialised version of its \AR{Fusion} counterpart stating that
...
Once we have \AF{𝓡}, we can concisely write down the constraint \ARF{varᴿ} which is also already
present in the definition of \AR{Fusion}.
...
respective \ARF{zro}s are producing related values. This will provide us with just the right
pair of related values to use in \AR{Fusion}'s \ARF{lamᴿ}.
...
the renaming to every single term in the substitution. We also depart from the use of \AF{Eqᴿ}
as the relation for values: indeed we now compare variables and terms. The relation \AF{VarTermᴿ}
relates variables and terms by wrapping the variable in a \AIC{`var} constructor and demanding
