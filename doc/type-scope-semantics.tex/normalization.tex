...
equation theory we are interested in. In this chapter, we will consider different
...
the computational part of such proofs. Such a construction starts by describing
a family of types \AB{Model} by induction on its \AF{Type} index. Two
procedures are then defined: the first (\AF{eval}) constructs an element
of (\AB{Model} \AB{σ} \AB{Γ}) provided a well typed term of the corresponding
...
of the model (\AB{Model} \AB{σ} \AB{Γ}). NBE composes the two procedures.


The definition of this \AF{eval} function is a natural candidate for our
\AF{Semantics} framework. We will see in this chapter that the various variations
on normalisation by evaluation that we will consider can all be described as
instances of the following \AR{NBE} interface: a \AR{Semantics} working on the
\AB{Model}, an embedding of variables into model values and a reification function
from model values to normal forms.
...
NBE is always defined \emph{for} a given equational
theory; we start by recalling the various
...
Thanks to \AF{Renaming} (cf. \cref{fig:synren}) and \AF{Substitution}
...
\caption{η-expansion and β-reduction in terms or \AF{th\textasciicircum{}Term} and \AF{sub}}
\end{figure}
