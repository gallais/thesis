...
sub^↝⋆ t ρᴿ = Simulation.sim sim ρᴿ t where
  ...
  sim : Simulation TermD Substitution Substitution ↝⋆ᴿ ↝⋆ᴿ
...
reify^𝓡 : ∀ σ τ (l : Tm TermD i τ (σ ∷ Γ)) (ρ : (Γ ─Env) Term Δ) →
  Kripkeᴾ 𝓡ᴾ 𝓡ᴾ (σ ∷ []) τ (Semantics.body Substitution ρ (σ ∷ []) τ l) →
...
case^𝓡 : (t : Term (σ + τ) Δ)
  ...
  Kripkeᴾ 𝓡ᴾ 𝓡ᴾ (σ ∷ []) ν (Semantics.body Substitution ρ (σ ∷ []) ν l) →
  Kripkeᴾ 𝓡ᴾ 𝓡ᴾ (τ ∷ []) ν (Semantics.body Substitution ρ (τ ∷ []) ν r) →
...
fundamental : Fundamental TermD Substitution 𝓡ᴾ 𝓡ᴾ
...
Fundamental.algᴾ fundamental = algᴾ where
  ...
  algᴾ : ∀ {s} (b : ⟦ TermD ⟧ (Scope (Tm TermD s)) σ Γ) →
          let v = fmap TermD (Semantics.body Substitution ρ) b in
          P.All 𝓡ᴾ _ ρ → Fdm.All TermD (Kripkeᴾ 𝓡ᴾ 𝓡ᴾ) v → Δ ⊢𝓡 σ ∋ Semantics.alg Substitution v
  -- case anlaysis
