...
module _ {I : Set} {T : I ─Scoped} where
  ...
  thBodyEnv :
    ...
    let σ : (Ξ ++ Γ ─Env) Var (Ξ ++ Δ)
        σ = freshˡ vl^Var Δ ++^Env th^Env th^Var ρᴬ (freshʳ vl^Var Ξ)
    in All Eqᴿ (Ξ ++ Γ) (select σ (ρ₄ ++^Env ρᴮ)) (ρ₅ ++^Env ρᴬᴮ)
  lookupᴿ (thBodyEnv {Γ} {Δ} {Θ} {Ξ} {ρᴬ} {ρᴮ} {ρᴬᴮ} {ρ₄} {ρ₅} ρᴿ vsᴿ) k
    with split Ξ k
  ... | inj₁ kˡ = begin
    lookup (ρ₄ ++^Env ρᴮ) (injectˡ Δ (lookup (base vl^Var) kˡ))
      ≡⟨ injectˡ-++^Env ρ₄ ρᴮ (lookup (base vl^Var) kˡ) ⟩
    ...
    lookup ρ₅ kˡ
      ∎
  ... | inj₂ kʳ = begin
    lookup (ρ₄ ++^Env ρᴮ) (injectʳ Ξ (lookup (base vl^Var) (lookup ρᴬ kʳ)))
      ≡⟨ injectʳ-++^Env ρ₄ ρᴮ (lookup (base vl^Var) (lookup ρᴬ kʳ)) ⟩
...
module _ {I : Set} {d : Desc I}  {𝓥 𝓒 : I ─Scoped}
  ...
  subBodyEnv :
    ...
    let σ : ((Ξ ++ Γ) ─Env) (Tm d _) (Ξ ++ Δ)
        σ = freshˡ vl^Tm Δ ++^Env th^Env th^Tm ρᴬ (freshʳ vl^Var Ξ)
    in All Eqᴿ (Ξ ++ Γ) (𝓢.semantics (ρ₄ ++^Env ρᴮ) <$> σ) (ρ₅ ++^Env ρᴬᴮ)
  lookupᴿ (subBodyEnv {Γ} {Δ} {Θ} {Ξ} ρᴬ {ρᴮ} {ρᴬᴮ} {ρ₄} {ρ₅} ρᴿ vsᴿ) k
    with split Ξ k
  ... | inj₁ kˡ = begin
    let t = ren (pack (injectˡ Δ)) (lookup (base vl^Tm) kˡ) in
    𝓢.semantics (ρ₄ ++^Env ρᴮ) t
      ≡⟨ cong (𝓢.semantics (ρ₄ ++^Env ρᴮ)) (sym (eq^quote _ t)) ⟩
    𝓢.semantics (ρ₄ ++^Env ρᴮ) (Fusion.reifyᴬ 𝓕 _ t)
      ≡⟨ Fusion.fusion 𝓕 (packᴿ (injectˡ-++^Env ρ₄ ρᴮ)) (lookup (base vl^Tm) kˡ) ⟩
  ...
  ... | inj₂ kʳ = begin
    ...
    eqᴿ : All Eqᴿ Δ (select (freshʳ vl^Var Ξ) (ρ₄ ++^Env ρᴮ)) ρᴮ
    lookupᴿ eqᴿ v = begin
      lookup (select (freshʳ vl^Var Ξ) (ρ₄ ++^Env ρᴮ)) v
        ≡⟨ injectʳ-++^Env ρ₄ ρᴮ (lookup (base vl^Var) v) ⟩
