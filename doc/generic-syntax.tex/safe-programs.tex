...
name generation, the same \AF{fresh} function and the same notions of \AF{Name}
and \AF{Printer} for the semantics' values and computations.
...
The key component making this work is the reification function \AF{reify\textasciicircum{}M}
turning the \AF{Kripke} spaces we get from the semantics framework into \AF{Pieces}. This
function has to be monadic so that we may generate fresh names. It uses the fact that
environments are traversable and that names are easily proven to be \AF{VarLike} to
...
Using the \AF{closed} version of the fundamental lemma of semantics and
