...
POPLMark Reloaded challenge (\citeyear{poplmark2}) where \AF{rensub} was actually
needed.
...
use in practice: \AF{quoteᴬ} which takes a Kripke function space and returns a
term by first feeding feeding placeholder values to the Kripke function space
and getting a \AB{𝓒ᴬ} and then reifying it thanks to \ARF{reifyᴬ}.
...
the recursive substructures are \AF{Kripkeᴿ}-reladed.
...
The proofs involve two functions we have not mentiond before: \AF{zip} maps a proof that
a property holds for any recursive substructure over a the arguments of constructor to
